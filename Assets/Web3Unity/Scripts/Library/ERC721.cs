...
public class ERC721
{
    ...
    public static async Task<string> TokenOfOwnerByIndex(string _chain, string _network, string _contract, string _account, int _index, string _rpc="")
    {
        string method = "tokenOfOwnerByIndex";
        string[] obj = { _account, _index.ToString() };
        string args = JsonConvert.SerializeObject(obj);
        string response = await EVM.Call(_chain, _network, _contract, abi, method, args, _rpc);
        return response; 
    }
