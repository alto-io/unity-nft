...
handlers.setInitialMMR = function (args, context) {
    const statistics = App.GetPlayerStatistics(currentPlayerId, [App.Statistics.MMR]);
    const statisticMMR = statistics.find(s => s.StatisticName === App.Statistics.MMR);
    if (App.IsNull(statisticMMR)) {
        var updates = [];
        updates.push({
            StatisticName: App.Statistics.MMR,
            Value: 1000,
        });
        App.UpdatePlayerStatistics(currentPlayerId, updates);
    }
}

function IncMMR(playfabId, delta) {
    ...
    App.UpdatePlayerStatistics(playfabId, updates);
}
...
handlers.playerLogin = function (args, context) {
    ...
    const statistics = App.GetPlayerStatistics(currentPlayerId, [App.Statistics.Level, App.Statistics.XP, App.Statistics.MMR]);
